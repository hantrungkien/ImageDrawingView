...
import com.kienht.imagedrawing.sticker.OnTouchDrawViewListener;
...
public class FreeDrawView extends View implements View.OnTouchListener, OnDrawBitmapAsyncCallback {
    ...
    private OnDrawBitmapEventListener mOnDrawBitmapEventListener;
    private OnTouchDrawViewListener mOnTouchDrawViewListener;
    ...
    public void setOnDrawCreatorListener(OnDrawBitmapEventListener mOnDrawBitmapEventListener) {
        this.mOnDrawBitmapEventListener = mOnDrawBitmapEventListener;
    }

    public void setOnTouchDrawViewListener(OnTouchDrawViewListener mOnTouchDrawViewListener) {
        this.mOnTouchDrawViewListener = mOnTouchDrawViewListener;
    }
    ...
    private TakeScreenShotAsyncTask mTakeScreenShotAsyncTask;

    /**
     * Create a Bitmap with the content drawn inside the view
     */
    public void getDrawBitmap() {
        if (mOnDrawBitmapEventListener == null) {
            return;
        }
        if (mTakeScreenShotAsyncTask != null) {
            mTakeScreenShotAsyncTask.cancel(true);
            mTakeScreenShotAsyncTask = null;
        }
        mTakeScreenShotAsyncTask = new TakeScreenShotAsyncTask(this, getWidth(), getHeight());
        mTakeScreenShotAsyncTask.execute();
    }
    ...
    public boolean onTouch(View view, MotionEvent motionEvent) {
        if (mOnTouchDrawViewListener != null && motionEvent.getAction() == MotionEvent.ACTION_UP) {
            mOnTouchDrawViewListener.onDrawViewTouched();
        }
        if (isLock) {
            return true;
        }
    ...
    @Override
    public void onDrawBitmapCreated(Canvas canvas, Bitmap bitmap) {
        if (mOnDrawBitmapEventListener != null) {
            draw(canvas);
            mOnDrawBitmapEventListener.onDrawCreated(bitmap);
        }
    }

    @Override
    public void onDrawBitmapError() {
        if (mOnDrawBitmapEventListener != null) {
            mOnDrawBitmapEventListener.onDrawCreationError();
        }
    }

    private static class TakeScreenShotAsyncTask extends AsyncTask<Void, Void, Void> {
        private final int mWidth, mHeight;
        private Canvas mCanvas;
        private Bitmap mBitmap;
        private OnDrawBitmapAsyncCallback mListener;

        public TakeScreenShotAsyncTask(@NonNull OnDrawBitmapAsyncCallback listener, int mWidth, int mHeight) {
        ...
        protected void onPostExecute(Void aVoid) {
            ...
            if (mListener != null) {
                mListener.onDrawBitmapCreated(mCanvas, mBitmap);
            }
        }
    }
}
