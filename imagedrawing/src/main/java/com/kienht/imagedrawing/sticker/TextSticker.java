...
public class TextSticker extends Sticker {
    ...
    static final String TAG = TextSticker.class.getSimpleName();

    static final long TIME_SHOW_CURSOR = 200L;
    ...
    private boolean isEnableEditMode = false;
    private boolean isVisibleCursor = false;
...
//        }
        ...
        if (isEnableEditMode && isVisibleCursor) {
            int line = staticLayout.getLineForVertical(canvas.getHeight());
            int chars = staticLayout.getLineEnd(line);
            float cursorX = staticLayout.getPrimaryHorizontal(chars) + 5;
            float cursorStartY = staticLayout.getLineTop(line);
            float cursorFromY = staticLayout.getLineBottom(line) + 5;

            canvas.save();
            canvas.concat(matrix);
            canvas.drawLine(cursorX, cursorStartY, cursorX, cursorFromY, cursorPaint);
            canvas.restore();
        }
    }
    ...
    void showCursor() {
        if (System.currentTimeMillis() - lastCursorChangeState > TIME_SHOW_CURSOR) {
            isVisibleCursor = !isVisibleCursor;
            lastCursorChangeState = System.currentTimeMillis();
        }
    }

    boolean isEnableEditMode() {
        return isEnableEditMode;
    }

    void setEnableEditMode(boolean enableEditMode) {
        this.isEnableEditMode = enableEditMode;
        this.isVisibleCursor = this.isEnableEditMode;
    }
